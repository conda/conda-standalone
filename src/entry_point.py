...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1].startswith("constructor"):
        ...
        p.add_argument(
            "--extract-conda-pkgs", action="store_true", help="path to conda prefix"
        )
        p.add_argument(
            "--extract-tarball", action="store_true", help="extract tarball from stdin"
        )
        p.add_argument("--make-menus", nargs="*", metavar="MENU_PKG", help="make menus")
        p.add_argument("--rm-menus", action="store_true", help="rm menus")
        ...
        if (args.make_menus is not None) or args.rm_menus:
            ...
            spec = importlib.util.spec_from_file_location(
                "constructor_utils", utility_script
            )
